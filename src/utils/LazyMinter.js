...
const SIGNING_DOMAIN_NAME = "DeHiveNFT-Voucher"
...
class LazyMinter {
  ...
  async createVoucher(tokenId, uri, minPrice = 0) {
    ...
    const domain = await this._signingDomain(tokenId)
    const types = {
      NFTVoucher: [
   ...
   */
  async _signingDomain(tokenId) {
    if (this._domain != null) {
      return this._domain
    }
    //const chainId = await this.contract.getChainID(tokenId)
    const chainId = 31337;
